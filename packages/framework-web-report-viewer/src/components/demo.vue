<template>
  <div>
    <div class="MsiteHeaderOutter">
        <div v-for="(tab,index) in tabs" :key="tab.id" :title="tab.text"
        @click="selectMode(tab.mode)"
        class="HeaderButton">
        <span>{{tab.text}}</span>
        </div>
        <div class="changeSize">
            宽度：<el-input-number  size="mini" v-model="boxSize.width" controls-position="right"  :min="200" label="宽度"></el-input-number>
            高度：<el-input-number size="mini"  v-model="boxSize.height" controls-position="right"  :min="200" ></el-input-number>
        </div>
    </div>
    <msite
      ref="msite_report"
